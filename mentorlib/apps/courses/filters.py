...
class CourseFilter(filters.FilterSet):
    resource = filters.ModelMultipleChoiceFilter(
        field_name="resource__id", to_field_name="id", queryset=Resource.objects.all()
    )
    ...
    def search_mentor(self, queryset, name, value):
        return queryset.filter(
            Q(mentor__first_name__icontains=value)
            | Q(mentor__last_name__icontains=value)
        )
